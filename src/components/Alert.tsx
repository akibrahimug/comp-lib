...
const variantStyles: Record<AlertVariant, { wrap: string; icon: string; title: string }> = {
...
export const Alert = forwardRef<HTMLDivElement, AlertProps>(function Alert(
  { variant = 'info', title, icon, onClose, className, tw, children, ...props },
  ref
) {
  ...
  return (
    <div
      ref={ref}
      role="alert"
      className={mergeTw('flex items-start gap-3 rounded-xl border p-4 text-sm', styles.wrap, className, tw)}
      {...props}
    >
      {showIcon && (
        <span className={mergeTw('mt-0.5 shrink-0', styles.icon)}>
          {icon ?? (
      ...
      <div className="min-w-0 flex-1">
        {title && <div className={mergeTw('font-semibold', styles.title)}>{title}</div>}
        {children && <div className={mergeTw(!!title && 'mt-0.5', 'leading-relaxed')}>{children}</div>}
      </div>
      {onClose && (
        <button
          type="button"
          onClick={onClose}
          aria-label="Dismiss"
          className={mergeTw('-mr-1 -mt-1 shrink-0 rounded-md p-1 opacity-70 transition hover:opacity-100', styles.icon)}
        >
