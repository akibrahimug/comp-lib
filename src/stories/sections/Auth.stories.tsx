...
import { LuxeStage, Container, Logo, Eyebrow, Icon, surface, accentBtn, ghostBtn, darkInput, withSourceBelow } from './_luxe';
...
export const Login: Story = {
  parameters: {
    sourceCode: `import { Button, Input, Checkbox, Alert, Icon } from '@kasoma/comp-lib';

<form onSubmit={(e) => { e.preventDefault(); setDone(true); }}>
  {done && <Alert variant="success" title="Signed in">Welcome back.</Alert>}

  <Button intent="ghost" tw={ghostBtn}><Icon name="google" /> Google</Button>
  <Button intent="ghost" tw={ghostBtn}><Icon name="github" /> GitHub</Button>

  <Input type="email" prefix={<Icon name="mail" />} placeholder="ada@company.com" />
  <PasswordInput value={pw} onChange={setPw} />   {/* Input + show/hide toggle */}

  <Checkbox /> Keep me signed in
  <Button type="submit" fullWidth size="lg" tw={accentBtn}>Sign in <Icon name="arrow" /></Button>
</form>`,
  },
  render: () => {
    const Demo = () => {
...
export const Signup: Story = {
  parameters: {
    sourceCode: `import { Button, Input, Checkbox, Progress, Badge, Alert, Icon } from '@kasoma/comp-lib';

const strength = Math.min(100, pw.length * 14);

<form onSubmit={(e) => { e.preventDefault(); setDone(true); }}>
  <Badge tw="bg-accent/10 text-accent">Free forever plan</Badge>

  <Input placeholder="Ada" />  <Input placeholder="Lovelace" />
  <Input type="email" prefix={<Icon name="mail" />} placeholder="ada@company.com" />
  <PasswordInput value={pw} onChange={setPw} />

  {pw.length > 0 && <Progress value={strength} variant={strengthVar} size="sm" />}

  <Checkbox checked={terms} onChange={() => setTerms((v) => !v)} /> I agree to the Terms
  <Button type="submit" fullWidth size="lg" tw={accentBtn} disabled={!terms}>Create account</Button>
</form>`,
  },
  render: () => {
    const Demo = () => {
...
export const Settings: Story = {
  parameters: {
    sourceCode: `import { Button, Input, Toggle, Avatar, Alert, Dialog, Icon } from '@kasoma/comp-lib';

{saved && <Alert variant="success" title="Saved">Your changes have been saved.</Alert>}

<section>  {/* Profile */}
  <Avatar size="2xl">AL</Avatar>
  <Button size="sm" tw={accentBtn}>Change photo</Button>
  <Input defaultValue="Ada Lovelace" /> <Input defaultValue="ada@analytical.engine" />
</section>
...
<Dialog.Root open={confirm} onOpenChange={setConfirm}>
  <Dialog.Overlay /><Dialog.Content>Delete workspace?</Dialog.Content>
</Dialog.Root>`,
  },
  render: () => {
    const Demo = () => {
