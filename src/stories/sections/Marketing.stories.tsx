...
const meta: Meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          'Marketing sections composed entirely from the component library and dressed in the runtime-themeable identity. Switch themes from the toolbar; the source for each section is shown below it.',
      },
    },
  },
};
...
export const Hero: Story = {
  parameters: {
    sourceCode: `import { Button, Badge, Eyebrow, Icon } from '@kasoma/comp-lib';

<Container className="text-center">
  <Chip tone="accent"><Icon name="sparkle" /> v1.4 — 28 components</Chip>

  <h1 className="font-display text-7xl">
    Interfaces that feel
    <span className="italic">quietly expensive.</span>
  </h1>

  <p className="text-fg-muted">A Tailwind-first, variant-driven React system.</p>

  <Button size="lg" tw={accentBtn}>Start building <Icon name="arrow" /></Button>
  <Button size="lg" intent="ghost" tw={ghostBtn}><Icon name="github" /> Star on GitHub</Button>
</Container>`,
  },
  render: () => (
    <LuxeStage>
...
export const Features: Story = {
  parameters: {
    sourceCode: `import { Eyebrow, Icon } from '@kasoma/comp-lib';

const features = [
  { i: 'layers', t: 'Variant engine', d: 'A tiny tv() composer drives every variant.' },
  { i: 'bolt',   t: 'Polymorphic',    d: 'Render as any element with the as prop.' },
  { i: 'shield', t: 'Accessible',     d: 'Focus traps + ARIA ship in the box.' },
  // …
];
...
<section>
  ...
  <div className="grid lg:grid-cols-3">
    {features.map((f) => (
      <article key={f.t}>
        <Icon name={f.i} className="text-accent" />
        <h3>{f.t}</h3>
        <p>{f.d}</p>
      </article>
    ))}
...
export const Pricing: Story = {
  parameters: {
    sourceCode: `import { Button, Badge, Toggle, Icon } from '@kasoma/comp-lib';

const [annual, setAnnual] = useState(true);

<div className="flex items-center gap-3">
  Monthly <Toggle checked={annual} onChange={() => setAnnual((v) => !v)} /> Annual
  <Badge tw="bg-accent/10 text-accent">Save 20%</Badge>
</div>

{tiers.map((t) => (
  <div key={t.name} className={t.featured ? 'border-accent' : 'border-edge/12'}>
    <h3>{t.name}</h3>
    <p className="text-5xl">{priceOf(t.monthly)}</p>
    <Button fullWidth tw={t.featured ? accentBtn : ghostBtn}>Choose {t.name}</Button>
    {t.features.map((f) => (
