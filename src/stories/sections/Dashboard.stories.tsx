...
/* ───────────────────────────── small pieces ───────────────────────────── */
/* Stat cards now come straight from the library: <Stat label value delta trend icon hint /> */
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Stat label="MRR" value="$48,210" delta="12.4%" trend="up" hint="vs last month" icon={<Icon name="wallet" />} />
        ...
        <Stat label="NPS" value="72" delta="6 pts" trend="up" hint="vs last month" icon={<Icon name="star" />} />
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <Stat label="Signups" value="1,204" delta="18%" trend="up" hint="this year" icon={<Icon name="users" />} />
        <Stat label="Conversion" value="4.6%" delta="0.8%" trend="up" hint="this year" icon={<Icon name="gauge" />} />
        <Stat label="Revenue" value="$48.2k" delta="12%" trend="up" hint="this year" icon={<Icon name="wallet" />} />
      </div>
...
export const Dashboard: Story = {
  parameters: {
    sourceCode: `import { Table, Stat, Badge, Avatar, Button, Pagination,
  Input, DropdownMenu, Popover, Dialog, Icon } from '@kasoma/comp-lib';

// stat cards — one line each
<Stat label="MRR" value="$48,210" delta="12.4%" trend="up" icon={<Icon name="wallet" />} />

// searchable, paginated data table
<Table.Root hoverable>
  <Table.Header><Table.Row><Table.Head>Customer</Table.Head>…</Table.Row></Table.Header>
  <Table.Body>
    {rows.map((c) => (
      <Table.Row key={c.email}>
        <Table.Cell><Avatar size="sm">{c.init}</Avatar> {c.name}</Table.Cell>
        <Table.Cell><Badge>{c.status}</Badge></Table.Cell>
        <Table.Cell>
          <DropdownMenu.Root align="end">
            <DropdownMenu.Trigger>⋯</DropdownMenu.Trigger>
            <DropdownMenu.Content>
              <DropdownMenu.Item onClick={() => setDetail(c)}>View details</DropdownMenu.Item>
            </DropdownMenu.Content>
          </DropdownMenu.Root>
        </Table.Cell>
      </Table.Row>
...
<Dialog.Root open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
  <Dialog.Overlay /><Dialog.Content>…</Dialog.Content>
</Dialog.Root>`,
  },
  render: () => {
    const App = () => {
